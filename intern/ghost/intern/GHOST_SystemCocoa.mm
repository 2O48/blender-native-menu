...
- (void)nativeBlenderMenuCommand:(id)sender
{
  ...
  if (command != nil) {
    system_cocoa_->handleNativeMenuCommand([command UTF8String]);
  }
}
...
{
  GHOST_Window *window = (GHOST_Window *)window_manager_->getActiveWindow();
  if (!window && !window_manager_->getWindows().empty()) {
    window = (GHOST_Window *)window_manager_->getWindows().front();
  }

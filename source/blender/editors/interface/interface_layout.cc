...
static std::string ui_native_menu_button_title(const uiBut *but)
{
  blender::StringRef title = ui_but_drawstr_without_sep_char(but);
  return title.is_empty() ? but->str : std::string(title.data(), title.size());
}

static const char *ui_native_menu_operator_context_id(const wmOperatorCallContext context)
{
  switch (context) {
...
static std::string ui_native_menu_operator_pystring(bContext *C, const uiBut *but)
{
  ...
  if (opptr == nullptr) {
    WM_operator_properties_create_ptr(&opptr_default, but->optype);
    opptr = &opptr_default;
  }
  ...
  for (uiItem *item : layout->items_) {
    if (item->type_ != uiItemType::Button) {
      ui_native_menu_items_from_layout(
          C, static_cast<uiLayout *>(item), items, depth, menu_stack);
      continue;
    }

    uiBut *but = static_cast<uiButtonItem *>(item)->but;
    if (but->flag & UI_HIDDEN) {
      continue;
    }

    if (ELEM(but->type, UI_BTYPE_SEPR, UI_BTYPE_SEPR_LINE, UI_BTYPE_SEPR_SPACER)) {
    ...
    if (MenuType *submenu = UI_but_menutype_get(but)) {
    ...
    blender::Vector<std::string> &menu_stack)
{
  ...
  uiBlock *block = UI_block_begin(C, nullptr, __func__, blender::ui::EmbossType::Pulldown);
